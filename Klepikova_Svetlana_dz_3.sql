...
CREATE TABLE  files_for_messages (
    ...
    FOREIGN KEY (messages_id) REFERENCES messages(id) ON UPDATE CASCADE ON DELETE CASCADE, -- Делаем внешний ключ, к какому сообщению относится файл
...
CREATE TABLE users_status (
    ...
    ); -- Связали статус с идентификатором пользователя. Столбец status_id выступает одновременно и первичным, и внешним ключом
    -- Один пользователь - один уникальный статус.
    -- У одного статуса тоже один пользователь. Связь 1-1.
